...
<body>
    ...
    <div class="container">
        ...
        <div class="row row-cols-2">
            <div class="col-12 col-sm-12 col-md-12 grid-dash">
                <card class="card-first-dash">
                    <h3>Pedidos Aceitos</h3>
                    @if($totalPedidosAceitos)
                    <p id="db-dados">{{ $totalPedidosAceitos }}</p>
                    @else
                    <p id="db-empty">Não há</p>
                    @endif
                </card>
                <card class="card-first-dash">
                    <h3>Pedidos Recusados</h3>
                    @if($totalPedidosRecusados)
                    <p id="db-dados">{{ $totalPedidosAceitos }}</p>
                    @else
                    <p id="db-empty">Não há</p>
                    @endif
            ...
            <div class="col-12 col-sm-12 col-md-12 grid-dash">
                <card class="card-first-dash">
                    ...
                    @if($totalPedidosRecebidos)
                    <p id="db-dados">{{ $totalPedidosRecebidos }}</p>
                    @else
                    <p id="db-empty">Não há</p>
                    @endif
                </card>
                <card class="card-first-dash">
                    ...
                    <p id="db-dados-agen">{{ $ultimoAgendamento->created_at->format('d/m/Y') }} - {{ $ultimoAgendamento->created_at->format('H:i') }}</p>
                    @else
                    <p>Nenhum agendamento até o momento.</p>
                    @endif
                </card>
            </div>
            <div class="col-12 col-sm-12 col-md-12 grid-dash">
                <card class="card-second-dash">
                    @if($plano && $plano->plano)
                    <div class="plan-card">
                        <h2>{{ $plano->plano->nome }}</h2>
                        <span>{{ $plano->plano->descricao }}</span>
                        <div class="etiquet-price"> Ativo desde {{ $plano->created_at->format('d/m/Y') }}</div>
                    </div>
                @else
                    <div class="plan-card-free">
                        <h2>Free</h2>
                        <span></span>
                        <div class="etiquet-price"> Adquira moedas ou um novo plano</div>
